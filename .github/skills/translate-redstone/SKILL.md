...
> **分块输出格式**：长稿分块时逐块派 subagent（prompt 按 [subagent-dispatch#派发配方](../subagent-dispatch/SKILL.md#派发配方) 组装），结果写 `_work/<视频名>/_humanize_results/chunk_<k>.txt`：每行 `段号|修订后译文`（可附改动点说明）。
