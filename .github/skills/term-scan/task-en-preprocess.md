...
4. **非语音事件标记剔除**（DownSub/YouTube 自动字幕特有）：语音文本中夹带的方括号非语音事件标记（`[laughter]`/`[clears throat]`/`[sighs]`/`[snorts]`/`[clears throat and snorts]` 等，**内嵌句首/句中/句尾或独立成 cue**）**不是对白内容**——语义识别后**剔除**：不翻译、不进 ASR 清单、不改时间码、不增删 cue；剔除后残留多余空格清理为正常间距；整条 cue 剔除后无语音文本 → **文本留空**（cue 与时间码保留）。**不硬编码枚举标记类型**（种类不确定），靠语义判断；无法确定 `[...]` 是否为标记（可能真是内容）→ **保持原文** + 登记 `[待审核]`
5. **`[Music]`/`[Applause]` 等纯标记 cue 保留不动**（整条 cue 去括号后无文本，作为时间骨架保留）
...
- **标准 SRT 格式**：OWNED 每个 cue = 一段三行 `段号` / `时间码 --> 时间码` / `修正后文本`，**前后 cue 之间必须空一行**（`srt_join_parts.py` 按空行切块解析，缺空行会丢 cue）
