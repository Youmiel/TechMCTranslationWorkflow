...
2. **未命中判断数据源**（wiki-tools 降级链）：Wiki 擅长类型（基础定义/合成配方/机制）→ `mc-wiki-fetch-mcp` 的 `get_page`（无损源），不可用按可靠度降级（`fetch_wiki.py` → `minecraft-wiki-mcp`）；社区类型（高端技术/经验/人名）→ 先查 `indexes/repos/` 索引定位本地仓库文件，不网络抓取
