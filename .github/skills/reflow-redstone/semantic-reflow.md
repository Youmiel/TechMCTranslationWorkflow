...
> 本文件 = `reflow-redstone` 阶段二（语义回填，semantic reflow）的**完整执行指令**（步骤 1-7，各步按「归一化 → 处理 → 校验」三段组织）。**仅进入阶段二时读取**——阶段〇/一由 [SKILL.md](SKILL.md) 指挥；阶段产物链 / 中断恢复路由 / 输出门禁见 [SKILL.md](SKILL.md#中间产物与断点恢复)。步骤号与 SKILL.md「中断恢复路由」对应。
