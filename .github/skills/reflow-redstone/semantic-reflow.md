# reflow 阶段二执行细节（semantic-reflow）

> 本文件 = `reflow-redstone` 阶段二（语义回填，semantic reflow）的**完整执行指令**（步骤 1-7，各步按「归一化 → 处理 → 校验」三段组织）。
> - **仅进入阶段二时读取**
> - 阶段〇/一由 [SKILL.md](SKILL.md) 指挥
> - 阶段产物链 / 中断恢复路由 / 输出门禁见 [SKILL.md](SKILL.md#中间产物与断点恢复)
> - 步骤号与 SKILL.md「中断恢复路由」对应
...
**反馈策略**（reflow 校验脚本统一，控制上下文占用）：默认只输出「问题数 + 提示」；主会话收集错误清单（task-fix 派发素材）时带 `--expand` 取每处明细；针对单块修复（`--chunk <k>`）只校验该块、默认展开该块详情，其他块报错零占用。
...
3. **补标点质量校验（逗号堆砌/超长句/断句稀疏/疑似可断句）**（任务：逐块按 `.?!` 分句，检测补标点质量）
  ...
  - 处理方式：
    - 硬命中 → 定点修复（B 档 `task-fix` 在语义断点补句末标点 `?!.`）
    - 软命中 → 主会话 / 用户复核确认语义断点后**并入 B 档 `task-fix` 清单一并修**（主会话不自行定点编辑）
    - 块内句数正常、句均正常（真实长句 ~500 字符内）→ 放行
...
> **路径选择（必须询问用户，不得默认）**：进入本步骤先向用户说明两条路径差异——5-1 = LLM 语义分句（精细、可表达子句级切分，token 大 ~35k/块）；5-2 = 脚本断句（LLM 只做整句级匹配、断句/填回全机械，省 token 但为近似对应，漏句留空标记 + 人工审核兜底，见 [task-match](task-match.md)）——**由用户选择** 5-1 或 5-2 后执行，不得自行默认（含中断恢复路由 9 从 5-1/5-2 续时同样先询问）。
...
1. **预分句 + ZH 机械化断句（脚本，方案 4——断句基线机械化）**：`python scripts/srt_reflow_presplit.py reflow/r01_results/ reflow/r02_results/ -o reflow/ --zh-list-out reflow/r03_zslim`
  ...
  - **`--zh-list-out reflow/r03_zslim`（独立产物路径，5-2 task-match 输入）**：额外生成整句级 Z 精简列表 `reflow/r03_zslim/chunk_<k>.txt`（每行 `Z<n> <整句文本>`，省 ~80% 脚手架）
    ...
    - 5-2 时 build-r03 填回仍读 r03_normalized_2（子句段机械切分）
    - 故 r03_normalized_2 与 r03_zslim 并存、各司其职，不相替代
...
1. **r03 统一校验**：`python scripts/srt_reflow.py check-r03 reflow/r03_results/ <01> reflow/r02_results/ --chunks reflow/chunks/ [--cjk-speed 5] [--no-frag] [--no-mismatch] [--full-warnings]`。
   - 逐块六查（锚定缩到块内 cue 区间、ZH 忠实缩到块内 r02）
   - **主会话统一跑，勿交 subagent**——与 subagent-dispatch 纪律母版「五、工作区与工具纪律」的「不运行全局校验脚本」同指一次运行
  ...
  - ⑧ **格式标记残留（跨块句硬违规 / 占位预警）**：
    - r03 扫描 `【承接句】` / `【延伸句】` 残留 → **硬违规打回**（跨块句标记应在衔接归位 / 翻译 / 预分句时消除；回填会把标记原样写进交付 SRT——uVOFckoMdIU chunk_002 S94 事故）
    - `[待审核: …]` 占位残留 → **存疑预警**（提示回 r02 定稿或人工确认，防止占位流进交付）
  - **处置（批量告警闭环，锁死 ≤3 轮）**：
    ...
    3. **一次复验**：task-fix 后重跑 check-r03
       - 残留增量 → 第二轮 B 档只派**增量残留清单**（逐轮收敛）
       - 连续多轮仍失败 → 升级 C 档整块重派
    4. **严禁全量重写**：禁止整块打回全文重写（token 反超现状，见 token 优化方案「五」)
    5. 存疑预警（⑤⑥⑦ + ⑧ 占位）：Agent 智能判断
       - ⑤⑥ 独立开关 `--no-frag` / `--no-mismatch`（预警过多、效果不佳时可单独关一项降噪）
       - **默认折叠**（统计 + 前 5 条样例，其余省略）；分句阶段需全量看预警时加 `--full-warnings`
