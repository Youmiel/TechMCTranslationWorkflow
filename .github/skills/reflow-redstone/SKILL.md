...
- **阅读舒适优先**：单条字幕以"读得舒服"为准（≤20 字、语义完整、节奏自然）；原轴只是时间骨架 + 真实 cue 边界参考，不因"尊重原轴"保留超长句
...
- **空隙探测前置（r01 硬性断句）**：步骤 1 前跑 `srt_reflow_gap_scan.py` → `r00_gaps.md` + `srt_reflow_breaks.py` → `r01_breaks.md`（断句点清单 + 注入【强制断句】标记输入，结构强制）；输出 `srt_reflow_check_breaks.py` 打回校验（违规退出码 1，受控例外 Agent 裁决）。软指令不足（S56 实证空隙被合句吞掉）
...
按 [redstone-preprocess](../redstone-preprocess/SKILL.md) 原样执行（阶段〇 预判 / §1.1 扫描 → `01_subtitle_asr_fixed.srt` / §1.2 补齐 / §1.3 确认 → `02_terms.md` / §1.4 入库）。`--cue-exact` 校验保留（保护原轴骨架）。
...
1. 把 `r01_merged_en.txt` 交给 LLM：不改变句子顺序（可合并/拆分，顺序不得颠倒）、保留全部标点、口语化、严格使用 `02_terms.md` 术语、输出完整连贯中文（不输出原文、不编号、不拆段）
2. 落盘 `reflow/r02_translation_zh.txt`
3. 分块策略（优先级）：整段优先 > 极长间隔语义段（gap > 5s 为界，注入术语表 + 前段末尾 1–2 句衔接）> N+M 兜底（`srt_chunk.py`）；语义段是步骤 1/4/5 统一分块单位
...
- **告警**：输出 `r04_alerts.md`——时长分布（超短 <300ms / 超长 >15s 或 >2×中位）、单元内 gap > 5s、剪辑跳转 > 10s、预测点清单、行宽 > 20
- 动作规则：碎片 cue 合并容纳完整单元；行宽 > 20 必切（"标点+有 cue"处，无 cue 则预测点）；时长超限不单独触发；[Music] 等非语音 cue 跳过；n:1 合句超长找"语义分割 + 有 cue"处切
...
按 [humanizer-zh](../humanizer-zh/SKILL.md) 独立上下文执行，只读 `r04` 全稿；红石术语译名不受影响。
