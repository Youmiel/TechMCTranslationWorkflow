...
- 当 task-fix 定点修 r03 拆句子单元（行宽重切）时 → 慎用——子单元拆分改动易破坏 EN/ZH 互斥拼接（task-fix 重写子单元时与整句对不齐），收敛慢；若同块错误密集（拆句互斥 + 行宽 + 锚定混合）→ 直接整块重派（C 档），因为定点修复在结构性问题前 token 反超、收敛不可控。（案例：uVOFckoMdIU chunk_002 前 2 轮 task-fix 修出 6 组新互斥破坏，拆半重派根治）
