...
- 当新视频所属**主题已处理过**（同一数据源系列 / 同一社区技术分支）时 → 先读该分支已在 `knowledge/01_terminology/` 建的专属 CSV（如 `shulker_farm.csv`），因为前一个视频的用户裁定译名可整批复用、大幅降低 L3 查证量。（案例：mv3OAZGKfs4 潜影贝反应堆 41 条术语中 32 条由本地库直接覆盖、仅 9 条走 L3，因同主题 uVOFckoMdIU 已积累 aggro 引擎/复制机制/传送点等 20+ 条）
...
- 当空隙点两侧文本呈「前 cue 末尾无句末标点 `.?!`」**且**「后 cue 首字母小写」时 → 先判为**源字幕切分缺陷（假空隙）**而非真实停顿，交用户确认后**整个排除**：不分块切块、不断句、跨它合并成一句，因为原字幕把同一句切成两条 cue 才空出这段时长，按停顿处理会一路放大成「注记归属错位」（空隙探测 → 强制断句 → 强制切块 → 补标点断句 → E 句被切开 → 旁注挂错句）。（案例：mv3OAZGKfs4 c88→c89 空 5.2s：前句 `...duplicates exactly once`（无句末标点）+ 后句 `over its lifetime.`（小写起首，字幕不会以小写开新 cue），用户 2026-09-21 判定为源缺陷；排除后该句归位为单条 E51）
- 当已排除某假空隙时 → 注意 `text_chunk.py --gaps` 是**自行探测**、**不读** `r00_gaps.md`，人工排除无法传达：分块须**去掉 `--gaps` 开关**（改普通单块），同时把 `r00_gaps.md` 该条标题改成脚本不可解析的格式（使 `check_breaks --gaps` 不再报违规），并把「必须跨该停顿合并」写进 `--prior-file` 注入补标点 subagent，因为这是当前唯一能表达「人工排除空隙」的三处通道。（案例：mv3OAZGKfs4 同上）
- 当作者旁注（`[note: ...]` / `[Note ...]`）因源字幕缺陷被排到后一句开头时 → 在**英中两侧同时归位**（英文把句号从注记前移到注记后使注记回到前句尾、中文把注记并入前句且塑形为中文语序），因为只改中文会造成新的 E 组错位、破坏 E 只读真值锚。（案例：mv3OAZGKfs4 `[note: this only applies for d < 5]` 被 E52 带走致中文注记挂到「反应堆达到平衡时」上）
...
- 当某条 cue **整条**是方括号包裹的作者旁注（如 `[Note that this is different from ... healed.]`）时 → 在 01 阶段**去掉外层方括号、保留正文**，因为 `is_pure_marker()`（去方括号后为空即判标记）会把它当 `[Music]` 类环境标记，`parse_srt` 随之剔除 → 该句不进任何 E 句、不参与回填，且**因下游一律剔除而无任何校验会报错（静默丢内容）**；判据是「括号内是成句的自然语言」而非 `[Music]`/`[Applause]` 那类单词标记，只能靠语义判断。（案例：mv3OAZGKfs4 c78 / c97 两条作者旁注，gap_scan 报「非语音标记 2 条已跳过」、改后正常进 E 句固化）
...
- 当用户要求在**冒号处**断句（如 `...it would be good to answer: / First: how do we...`）时 → 在 r02 把译文改写成**两句、句尾用句号**（`不过有几个问题值得回答一下。`），因为中文 Z 句界集合只有 `。！？…`、**冒号作不了句界**，保留冒号就不可能在此处断开（二者只能选一，需向用户说明）。（案例：mv3OAZGKfs4 00:04:21 用户指定，措辞因此变化、用户接受）
