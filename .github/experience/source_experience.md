...
- 当回填打包时 → 句末标点（。！？…）是**显示段硬边界**（逐句独立成段，禁为凑宽度跨句拼合），否则 `pack_candidates` 会把「A。B，」拼进同一行（一行内夹句号，用户明确否决）；句内超宽才按断点强度拆（2026-09-22 起由 `srt_reflow_punct` 角色表统一：`terminator` 句界 / `rparen` / `strong` `；：—` / `clause` `，` / `list` `、`）。（案例：edkkLsir9M8 用户裁定；ZXGpmaIcMMo 两段跨句粘连，加句末级后分别断为 2 段 / 3 段）
- 当遇到 `1.` `2.` 这类「数字 + 点」时 → **不要加序号保护**，按**句末点**处理：`1. 先建地基` 与 `takes 5. Again` 形态完全相同（数字 + 点 + 空格 + 内容），局部字符无法区分；实测 6 个真实视频回归中 `数字.` 作句末点多次、作列表项 0 次。但**无歧义形式要保护**：`1.)`（数字 + 点 + 右括号）——句末点后不可能紧跟右括号，旧实现曾把 `1.)` 误切成 `1.` + `)`。（2026-09-22 回归实证；`2）` `1、` `①、` 的 `)` `、` 不在句界集合内，本就无需 guard）
- 当句子含括注（`…（当然，如果布线更糟那就另说）`）且整体超宽时 → 括注内断点须为**软代价**（`BRACKET_BREAK_PENALTY=6.0`：> `clause` 5.0 使「宁可有逗号断点也不轻易破括注」、< 碎片罚 20.0 使「宁可破括注也不制造碎片」），因为**硬禁止**会让整块无法切分、直接超硬限 26。（2026-09-22 端到端回归实证：LyU6a4PuDjo 一处 28 字硬超限即此因）
...
- 当用户要求「分号前后内容隔开」（并列项不挤在同一屏段）时 → **语义已变更为「分号 = `strong` 强断点、可被宽度否决」**（2026-09-22 断句/拼句角色化重构）：拼合改**代价最小化**（`strong` 代价 2.0 ≪ `clause` 5.0），分号处通常优先断开，但**并列项各自过短时允许合回一段**（旧「分号硬断」会切出 `A；B；C` 均宽 8 三段孤立，已取消该专有路径；函数 `pack_no_cross_semicolon` 保留但等价于普通拼合）。（原裁定 xh511sviyXc 2026-09-17；变更依据 = 用户 2026-09-21 定调「拼合应让断点强度参与决策、可被宽度否决」）
- 当要决定「超宽句在哪断开」时 → 用 `scripts/srt_reflow_punct.py` 的**标点功能角色表**（强度降序 `terminator`/`rparen`/`strong`/`clause`/`list`）+ `pack_by_strength`（代价 = 断点弱度 + 段宽偏离 [15,22] + 碎片罚），**勿用「贪心填满 hard_max」**——贪心会吞掉强断点、保留弱断点（实证：`不过有几个问题值得回答一下：第一，`(17)+`怎么搭…？`(12) 断在逗号；代价最小化给出 `…回答一下：`(14)+`第一，怎么搭…？`(15)）。括号是**软优先**（不是硬禁止）：硬禁止会使 `…中继器（当然，如果布线更糟那就另说）`(28) 无法切分而超硬限（2026-09-22 端到端回归实证）
...
- 当已排除某假空隙时 → **改 `r00_gaps_active.tsv` 该行 `status` 为 `excluded`**（2026-09-22 起的正式通道）：分块用 `text_chunk.py --gaps-file <tsv>`（`excluded` 自动跳过）、校验用 `check_breaks --gaps <tsv>`（自动跳过）、补标点先验用 `--prior-file` 注入「必须跨该停顿合并成一句」。**重跑 `gap_scan` 不覆盖人工决定**。⚠️ **旧做法已废弃**（当时无正式通道，只能用三处 hack：① 分块去掉 `--gaps` 开关；② 把 `r00_gaps.md` 该条标题改成脚本不可解析格式以骗过 `check_breaks` 正则；③ `--prior-file` 注入）——勿再重复。（案例：mv3OAZGKfs4；2026-09-21 重构后已收口为 tsv 单一事实源）
- 当空隙两侧文本呈「前 cue 末尾无句末标点 `.?!`」**且**「后 cue 首字母小写」时 → `srt_reflow_gap_scan.py` **现已自动标记为「疑似源切分缺陷」**（`kind=suspect`，报告单独分节 + tsv 标注），**无需人工先发现**；默认仍生效（脚本只报告不改行为），确认后改 tsv `status=excluded`。（判据两条同时成立才有高置信；案例：mv3OAZGKfs4 c88→c89 空 5.2s：前句 `...duplicates exactly once` + 后句 `over its lifetime.` 小写起首）
