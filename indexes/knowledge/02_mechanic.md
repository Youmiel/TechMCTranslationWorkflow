...
- **`subtitle-number-style.md`** — 字幕数字体例：数值与分数用阿拉伯数字（`9 个格子`、`1/3`、`2 档信号强度`）、千分位**不加逗号**（`72000`）；序数词与虚指保留中文（`第一格`、`一件物品`、`一种`）；同一概念的单位词须前后统一（不可「3 格」与「2 档」并存） [通用]
  - 关键词：数字体例, number style, 阿拉伯数字, 中文数字, 千分位, 分数, 序数词, 虚指, 单位词统一, 字幕书写
  - 来源：用户裁定（2026-09-18，ZXGpmaIcMMo PRR 5）
