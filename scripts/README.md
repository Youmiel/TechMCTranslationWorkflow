...
| `srt_reflow2_backfill.py` | **继承回填（reflow2 步骤 7）**：读 `zh_sentences/`（Z 文本）+ `align/`（Z组=E组，task-match LLM 产物）+ `en_timeline/`（E 固化时间）——每 Z 组继承 E 组覆盖时间（源头固化、天然零重叠）；Z 超宽（>26）按**标点功能角色表**拆子段 + 按阅读速度比例在 E 区间内细分（复用 `allocate._allocate_by_weight`：吸附真实 cue 边界 ≤300ms，无则 100ms 预测点）——拼合 = 代价最小化（分号自 2026-09-21 起由硬断降为 `strong` 强优先，可被宽度否决）；同步生成 `r04_bilingual.srt` + `r04_alerts.md`（🔪 长句碎片/⏱️ 独立短句/🎯 预测点）；退出码 1 = 对齐不完整 | `python scripts/srt_reflow2_backfill.py <zh_sentences/> <align/> <en_timeline/> -o reflow2/r04_draft.srt [--alert reflow2/r04_alerts.md] [--bilingual reflow2/r04_bilingual.srt]` |
