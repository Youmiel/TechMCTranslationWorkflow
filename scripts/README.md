...
| `srt_reflow_presplit.py` | 预分句标号（方案 3，步骤 5 分句输入）：EN 按句末标点 `.?!`（`r01_results/`）→ `r03_normalized_1/`（E1..En）；ZH 按句号 `。！？`（`r02_results/` 直读原稿）→ `r03_normalized_2/`（Z1..Zm）；缩写/省略号/括号配平保护；折行合并保留中英/数字空格（`Carpet 的 fillUpdates`、`139 147 11`）；不形成中英对照，预分句为初分骨架 | `python scripts/srt_reflow_presplit.py <r01_results/> <r02_results/> -o reflow/` |
